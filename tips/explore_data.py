...
def data_set_study(check_corr_matrix=False, 
    ...
    if check_box_plot == True :
        ...
        plot_box_sns = sns.countplot(x='payment_type', data=train)
        plot_box_sns.set_title('Distribution of Payment Type', fontsize=18)
        fig = plot_box_sns.get_figure()
        fig.savefig(f"{PATH_Plots}/Payment_Type_Distribution.jpeg", format='jpeg') 
                
    if check_histogram == True:
        columns_list = [
            "VendorID", "passenger_count", "payment_type", 
            "tip_amount", "trip_distance_miles"
        ]
